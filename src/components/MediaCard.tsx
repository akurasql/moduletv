...
export function MediaCard({ item, rank }: MediaCardProps) {
  ...
  return (
    <Link
      ...
      className="group relative flex-none w-[110px] sm:w-[130px] md:w-[140px] lg:w-[150px] transition-transform duration-300 ease-apple-ease hover:scale-105 hover:z-30 cursor-pointer"
    >
      <div className="relative aspect-[2/3] w-full rounded-xl overflow-hidden bg-apple-darkGray shadow-md group-hover:shadow-2xl transition-all duration-300">
        ...
        <div className="absolute inset-0 border border-white/0 group-hover:border-white/15 rounded-xl transition-all pointer-events-none duration-300" />
      </div>

      {rank !== undefined && (
        <div className="absolute -left-3 bottom-0 z-10 select-none pointer-events-none">
          <span className="text-[55px] md:text-[75px] font-black leading-none text-apple-black drop-shadow-[0_2px_8px_rgba(255,255,255,0.25)] stroke-white/10">
            {rank}
          </span>
        </div>
      )}
